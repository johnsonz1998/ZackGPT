...
from app.logger import debug_info, debug_success, debug_error
...
class ThreadManager:
    ...
    def create_thread(self, title: str) -> Thread:
        ...
        debug_success(f"Created thread: {thread.title}", {"thread_id": thread.id})
        return thread
    ...
    def add_message(self, message: ChatMessage) -> None:
        ...
        if message.thread_id in self.threads:
            ...
            thread.message_count = len(self.messages[message.thread_id])
    ...
    def delete_thread(self, thread_id: str) -> bool:
        if thread_id in self.threads:
            ...
            debug_info(f"Deleted thread: {thread_id}")
            return True
        return False
...
async def send_message(thread_id: str, request: SendMessageRequest):
    ...
    try:
        ...
        thread_manager.add_message(assistant_message)
...
async def get_system_stats():
    ...
    try:
        ...
        return SystemStats(
            ...
            total_messages=sum(len(messages) for messages in thread_manager.messages.values()),
            evolution_stats=evolution_stats,
            memory_stats={"status": "available"}  # TODO: Add real memory stats
        )
    except Exception as e:
